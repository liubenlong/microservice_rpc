...
import com.freedom.rpc.thrift.common.extended.client.ClientSideProcessorFetcherHelper;
import com.freedom.rpc.thrift.common.extended.client.ClientSideTMultiplexedProtocol;
...
public class SocketUtils {
	...
	public static TServiceClient getAopObject(Class clz) throws Exception {// HelloWorldService.Client
		//
		// 1)获取线程变量,避免 GC
	...
	private static TMultiplexedProtocol getProtocol(Class clz) throws Exception {
		...
		TSocketPool pool = MachineResource.getInstance().getSocketPool(group, service, edition);// pool
		if (null == pool) {
